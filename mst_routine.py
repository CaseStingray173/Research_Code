...
def mst_routine(current_node):
    cost = numpy.zeros([len(current_node), len(current_node)])
    for i in range(len(current_node)):
        for j in range(i, len(current_node)):
            temp = (current_node[j][0] - current_node[i][0]) * (current_node[j][0] - current_node[i][0]) + \
                   (current_node[j][1] - current_node[i][1]) * (current_node[j][1] - current_node[i][1])
